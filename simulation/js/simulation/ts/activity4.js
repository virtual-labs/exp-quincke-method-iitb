...
function activity4() {
    ...
    pp.addoffcanvas(3);
    //load_chart_data();
    pp.showtitle(`<p id="exp-title" style='width: 23vw;'>Calculate</span><p>`, 3);
...
function load_table4() {
    ...
    for (let i = 0; i < obs_table.length; i++) {
        obs_table[i].splice(3, 0, Math.pow(obs_table[i][2], 2));
        obs_table[i].splice(6, 0, (obs_table[i][4] + obs_table[i][5]));
    }
    let tab = new Verify_Rows_Cols_Custom_Fixed(header, obs_table, [0, 1, 2, 3, 4], [[3, 6], [3, 6], [3, 6], [3, 6], [3, 6]], '', parent, true, true, after_table4_verification, 5);
    tab.load_table();
}
function after_table4_verification() {
    let ele0 = document.getElementById('a4-1');
    let ele1 = document.getElementById('t-4');
    ele0.style.display = 'block';
    ele1.style.display = 'none';
    pp.showtitle(`<p id="exp-title" style='width: 23vw;'>Calculate susceptibility</span><p>`, 3);
...
function act4_verify() {
    ...
    let val3 = (document.getElementById('x-inp'));
